...
from cgan_pytorch.models.discriminator import discriminator_for_CIFAR100
...
from cgan_pytorch.utils.datasets import SuperCIFAR100  
...
def main_worker(ngpus_per_node, args):
    ...
    discriminator = discriminator_for_CIFAR100(args.image_size, args.channels)
    ...
    fixed_conditional = torch.randint(0, 20, (args.batch_size,))
    ...
    dataloader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=(sampler is None), pin_memory=True, sampler=sampler, num_workers=args.workers)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i,  (images, super_labels, _)  in enumerate(dataloader):
            ...
            real_loss = adversarial_criterion(discriminator(images, super_labels), valid)
            real_score = discriminator(images, super_labels)

            noise = torch.randn([args.batch_size, 100])
            conditional = torch.randint(0, 20, (args.batch_size,))
            ...
            # Train with real.
            real_output = discriminator(images, super_labels)
            print("Real output size: ", real_output.size())
            
            d_loss_real = adversarial_criterion(real_output, valid)
            
            d_loss_real.backward()
            d_x = real_output.mean()

            # Train with fake.
            fake_images = generator(noise, conditional)
            fake_output = discriminator(fake_images.detach(), conditional)
            print("fake_output shape:", fake_output.shape)
            print("fake_images shape:", fake_images.shape)
            fake = torch.zeros((fake_output.size(0), 1), device=device)
            print("fake shape:", fake.shape)

            d_loss_fake = adversarial_criterion(fake_output, fake)
            ...
            g_loss = adversarial_criterion(fake_output, valid)
